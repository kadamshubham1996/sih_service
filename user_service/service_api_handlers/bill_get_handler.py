...
def get_bill_for_user(auth_key,request_data):
    ...
    pending_bills = BillingEntry.objects.filter(user=login_entry.user,month=month,is_paid=False)
    # last_bill = None
...
def get_bill_for_bot(auth_key):
    login_entry = LoginEntry.objects.get(auth_token=auth_key)
    pending_bills = BillingEntry.objects.filter(user=login_entry.user, is_paid=False).last()
